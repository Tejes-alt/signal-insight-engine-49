...
export const Route = createFileRoute("/_authenticated/admin/integrations")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Administrator-only diagnostics for platform integrations, database and background sync.",
      },
      { name: "robots", content: "noindex" },
      { property: "og:title", content: "Integration diagnostics · SocialPulse admin" },
      { property: "og:description", content: "Administrator-only integration and pipeline diagnostics." },
  ...
  return (
    <div className="flex items-start gap-3 border-b border-border/60 py-3 last:border-0">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground">{ok ? "Configured" : (hint ?? "Configuration required")}</p>
      </div>
    </div>
  );
}
...
function AdminIntegrationsPage() {
  const { orgId, isAdmin } = useDashboard();
  ...
  if (!isAdmin) {
    return (
      <AppShell>
        <PageHeader title="Restricted area" description="This page is only available to workspace administrators." />
        <div className="panel flex items-center gap-3 p-6 text-sm text-muted-foreground">
          <Lock className="size-4" /> You don't have access to this page.
        </div>
      </AppShell>
    );
  }
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        description="Administrator-only. Technical configuration state for this installation — never shown to customers. Secret values are never displayed."
      />
      {query.isLoading ? (
        <div className="panel flex items-center gap-3 p-6 text-sm text-muted-foreground">
        ...
        <div className="grid gap-5 lg:grid-cols-2">
            ...
            {query.data.configuredCount > 0 ? (
              <div className="panel animate-rise flex items-center gap-3 border-success/40 p-4 text-sm">
                ...
                <span className="font-medium">
                  {query.data.configuredCount} of {query.data.integrations.length} platform integrations configured
                </span>
              </div>
            ) : (
              <div className="panel animate-rise flex items-start gap-3 border-warning/40 p-4 text-sm">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground">
                    No platform developer credentials are present on this installation yet. End users only
                    see a generic "temporarily unavailable" message.
                  </p>
                </div>
              </div>
            )}
          ...
          <section className="panel animate-rise p-6 lg:col-span-2">
        ...
        <div className="panel p-6 text-sm text-destructive">Diagnostics are unavailable right now.</div>
      )}
    </AppShell>
  );
}
