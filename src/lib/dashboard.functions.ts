 ...
 * Otherwise the bundle is assembled entirely from stored, synchronized data —
 * the dashboard never calls a social API during a render.
 */
export const getDashboard = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<AnalyticsBundle> => {
    ...
    const { buildRealBundle } = await import("./services/social.server");
    return buildRealBundle(data.orgId, data.rangeDays);
  });
